...
async function versionManagementExample() {
  ...
  const blogPost = await contentManager.createBlogPost({
    ...
    content: `
      ...
      \`\`\`
    `,
    excerpt: 'Learn how to get started with TypeScript for better JavaScript development.',
    authorId: 'author-123'
  });
  ...
  const v2 = await versionManager.createVersion(
    blogPost.blogPost.id,
    {
      ...
      \`\`\`
      `,
      excerpt: 'A complete guide to getting started with TypeScript, including basic types and setup.'
    },
    {
  ...
  const branchVersion = await versionManager.createVersion(
    blogPost.blogPost.id,
    {
      ...
      \`\`\`
      `,
      excerpt: 'Master TypeScript from basic concepts to advanced features like generics and interfaces.'
    },
    {
  ...
  const seoVersion = await versionManager.createVersion(
    blogPost.blogPost.id,
    {
      ...
      `,
      excerpt: 'Complete 2024 guide to TypeScript with benefits, use cases, and practical examples.',
      focusKeyword: 'TypeScript guide',
      keywords: ['TypeScript', 'JavaScript', '2024 guide', 'programming']
    },
    {
      branchName: 'feature/seo-optimization',
      changeSummary: 'SEO optimization with keyword targeting and benefits section'
    }
  );
  ...
  console.log('- Similarity:', (comparison1.similarityScore * 100).toFixed(1) + '%');
  ...
  console.log('- Similarity:', (comparison2.similarityScore * 100).toFixed(1) + '%');
  ...
  console.log('Rolled back to SEO version. New version:', rollbackResult.version);
  ...
  allVersions.forEach((version, index) => {
    ...
    console.log(`   Words: ${version.wordCount}, SEO Score: ${version.seoScore?.toFixed(1) || 'N/A'}`);
  ...
  branches.forEach(branch => {
    ...
    branch.versions.forEach(version => {
      console.log(`   └── ${version.version} (${version.createdAt.toISOString()})`);
    });
  ...
  for (const branchInfo of collaborativeBranches) {
    ...
    console.log(`Created collaborative branch: ${branchInfo.name} by ${branchInfo.author}`);
  }
