...
async function comprehensiveMultiPlatformExample() {
  ...
  try {
    ...
    const blogPost: BlogPost = {
      title: "The Evolution of E-commerce: From Storefronts to Digital Experiences",
      content: `
      ...
      `,
      excerpt: "Explore how e-commerce has evolved from basic online stores to sophisticated digital experiences, and discover the platforms and strategies driving modern online business success.",
      tags: ['e-commerce', 'digital transformation', 'web design', 'online business', 'technology', 'shopify', 'webflow'],
      keywords: ['e-commerce evolution', 'digital commerce', 'online business trends', 'e-commerce platforms', 'web design'],
      author: 'Digital Strategy Team',
      authorEmail: 'strategy@company.com',
      publishedAt: new Date(),
      status: 'published',
      seoTitle: "E-commerce Evolution: From Storefronts to Digital Experiences | 2024 Guide",
      seoDescription: "Discover how e-commerce has transformed from simple online stores to sophisticated digital ecosystems. Learn about modern platforms, trends, and strategies.",
      ...
      images: [
        {
          ...
          altText: 'Modern e-commerce interface showing multiple devices and shopping experience',
          caption: 'E-commerce has evolved into sophisticated digital experiences'
        }
      ]
    };
    ...
    console.log('Success rate:', `${publishResult.successCount}/${publishResult.successCount + publishResult.failureCount}`);
    ...
    console.log('- Average engagement rate:', `${(analytics.avgEngagementRate * 100).toFixed(2)}%`);
    ...
    Object.entries(analytics.platformBreakdown).forEach(([platform, data]) => {
      ...
      console.log(`  - Engagement rate: ${(data.engagementRate * 100).toFixed(2)}%`);
    });
    ...
    if (analytics.recommendations.length > 0) {
      ...
      analytics.recommendations.slice(0, 3).forEach(rec => {
        console.log(`${rec.platform}: ${rec.recommendation} (Priority: ${rec.priority})`);
      });
    }
    ...
    comparative.comparisons.forEach(comp => {
      ...
      console.log(`${comp.metric}: ${comp.winner} leads with ${winner} (${comp.difference}% difference)`);
    });
...
async function platformFactoryExample() {
  ...
  const capabilities = ['supportsScheduling', 'supportsAnalytics', 'supportsImages', 'supportsTags'];
  
  console.log('Platform'.padEnd(15) + capabilities.join(''.padEnd(5)).padEnd(60));
