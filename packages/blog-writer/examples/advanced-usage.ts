...
async function advancedBlogGeneration() {
  try {
    ...
    console.log(
      `✅ Research completed: ${research.keywords.primary.length} primary keywords found`,
    );
    ...
    const blogResult = await generateBlog({
      ...
      tone: 'authoritative',
      audience: research.overview.summary.includes('professional')
        ? 'professionals'
        : 'general',
      research: {
    ...
    console.log(
      `✅ SEO analysis completed: Score ${seoResult.analysis.score}/100`,
    );

    if (seoResult.optimizedPost) {
      console.log(
        `🔧 Applied ${seoResult.optimizationsApplied.length} optimizations`,
      );
    }
...
async function generateContentSeries() {
  try {
    ...
    const seriesStructure = [
      {
        template: 'guide',
        topic: 'Digital Marketing Fundamentals: A Complete Guide',
        keywords: ['digital marketing basics', 'marketing fundamentals'],
      },
      {
        template: 'howto',
        topic: 'How to Create Your First Digital Marketing Strategy',
        keywords: ['marketing strategy', 'digital marketing plan'],
      },
      {
        template: 'listicle',
        topic: '15 Essential Digital Marketing Tools Every Marketer Needs',
        keywords: ['marketing tools', 'digital marketing software'],
      },
      {
        template: 'comparison',
        topic:
          'Google Ads vs Facebook Ads: Which Platform Delivers Better ROI?',
        keywords: ['google ads', 'facebook ads', 'advertising comparison'],
      },
      {
        template: 'case-study',
        topic: 'How a Startup Grew 500% Using Content Marketing',
        keywords: ['content marketing case study', 'startup growth'],
      },
    ];
    ...
    for (let i = 0; i < seriesStructure.length; i++) {
      ...
      console.log(
        `📄 Generating article ${i + 1}/${seriesStructure.length}: ${item.topic}`,
      );
      ...
      const result = await generateBlog({
        ...
        keywords: [
          ...item.keywords,
          ...mainResearch.keywords.primary.slice(0, 2).map(k => k.keyword),
        ],
      ...
      console.log(
        `✅ Part ${i + 1} completed (${result.metadata.wordCount} words)`,
      );
    }

    console.log(
      `🎉 Content series completed: ${seriesResults.length} articles generated`,
    );

    // Generate series overview
    const totalWords = seriesResults.reduce(
      (sum, item) => sum + item.result.metadata.wordCount,
      0,
    );
    const avgScore =
      seriesResults.reduce(
        (sum, item) => sum + (item.result.blogPost.suggestions?.length || 0),
        0,
    ...
    console.log(
      `   Templates used: ${[...new Set(seriesResults.map(r => r.template))].join(', ')}`,
    );
      ...
      structure: [
        {
          ...
          prompt:
            'Create an exciting announcement about the new product launch.',
...
async function batchSEOAnalysis() {
  try {
    ...
    analyses.forEach(({ post, analysis, index }) => {
      ...
      console.log(
        `   Critical Issues: ${analysis.recommendations.filter(r => r.type === 'critical').length}`,
      );
      console.log(
    ...
    console.log(
      `⚠️  Needs improvement: "${worstPost.post.metadata.title}" (${worstPost.analysis.score}/100)`,
    );
