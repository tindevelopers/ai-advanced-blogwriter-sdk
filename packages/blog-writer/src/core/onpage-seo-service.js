...
exports.OnPageSEOService = void 0;
const ai_1 = require("ai");
const zod_1 = require("zod");
const seo_engine_1 = require("../types/seo-engine");
// Zod schemas for structured AI responses
const TitleOptimizationSchema = zod_1.z.object({
    analysis: zod_1.z.object({
        keywordPresence: zod_1.z.boolean(),
        keywordPosition: zod_1.z.number(),
        readabilityScore: zod_1.z.number().min(0).max(100),
        clickworthinessScore: zod_1.z.number().min(0).max(100),
        lengthOptimal: zod_1.z.boolean(),
        powerWords: zod_1.z.array(zod_1.z.string()),
        emotionalHooks: zod_1.z.array(zod_1.z.string()),
        suggestions: zod_1.z.array(zod_1.z.string()),
        optimizedVersions: zod_1.z.array(zod_1.z.string())
    })
});
const ContentAnalysisSchema = zod_1.z.object({
    keywordAnalysis: zod_1.z.array(zod_1.z.object({
        ...
        overOptimized: zod_1.z.boolean(),
        naturalUsage: zod_1.z.boolean()
    })),
    structure: zod_1.z.object({
        paragraphs: zod_1.z.number(),
        averageParagraphLength: zod_1.z.number(),
        sentences: zod_1.z.number(),
        averageSentenceLength: zod_1.z.number(),
        hasIntroduction: zod_1.z.boolean(),
        hasConclusion: zod_1.z.boolean(),
        listsCount: zod_1.z.number(),
        hasTableOfContents: zod_1.z.boolean(),
        readabilityScore: zod_1.z.number().min(0).max(100)
    }),
    topicCoverage: zod_1.z.object({
        mainTopics: zod_1.z.array(zod_1.z.string()),
        relatedTopics: zod_1.z.array(zod_1.z.string()),
...
const HeadingAnalysisSchema = zod_1.z.object({
    structure: zod_1.z.array(zod_1.z.object({
        level: zod_1.z.number().min(1).max(6),
        text: zod_1.z.string(),
        keywordPresence: zod_1.z.boolean(),
    ...
    analysis: zod_1.z.object({
        ...
        h1Text: zod_1.z.string().optional(),
        properHierarchy: zod_1.z.boolean(),
        keywordOptimization: zod_1.z.number().min(0).max(100),
        suggestions: zod_1.z.array(zod_1.z.string()),
        missingLevels: zod_1.z.array(zod_1.z.number()).optional()
    })
});
const LinkAnalysisSchema = zod_1.z.object({
    internal: zod_1.z.object({
        totalLinks: zod_1.z.number(),
        uniqueLinks: zod_1.z.number(),
        anchors: zod_1.z.array(zod_1.z.string()),
        suggestions: zod_1.z.array(zod_1.z.object({
            anchor: zod_1.z.string(),
            targetTopic: zod_1.z.string(),
    ...
    external: zod_1.z.object({
        totalLinks: zod_1.z.number(),
        uniqueDomains: zod_1.z.number(),
        suggestions: zod_1.z.array(zod_1.z.object({
            anchor: zod_1.z.string(),
            targetDomain: zod_1.z.string(),
            relevance: zod_1.z.number().min(0).max(100),
            authority: zod_1.z.number().min(0).max(100),
            reason: zod_1.z.string()
        })),
        quality: zod_1.z.number().min(0).max(100)
    }),
    anchor: zod_1.z.object({
        distribution: zod_1.z.array(zod_1.z.object({
            text: zod_1.z.string(),
            count: zod_1.z.number(),
            type: zod_1.z.enum(['exact', 'partial', 'branded', 'generic', 'naked'])
        })),
        overOptimized: zod_1.z.array(zod_1.z.string()),
        suggestions: zod_1.z.array(zod_1.z.string())
    })
});
const ImageOptimizationSchema = zod_1.z.object({
    analysis: zod_1.z.array(zod_1.z.object({
        src: zod_1.z.string(),
        issues: zod_1.z.array(zod_1.z.object({
            type: zod_1.z.enum(['missing_alt', 'poor_alt', 'oversized', 'wrong_format', 'not_optimized']),
            severity: zod_1.z.enum(['low', 'medium', 'high']),
            description: zod_1.z.string()
        })),
        suggestions: zod_1.z.array(zod_1.z.string()),
        optimizedAlt: zod_1.z.string().optional(),
        seoScore: zod_1.z.number().min(0).max(100)
    })),
    summary: zod_1.z.object({
        totalImages: zod_1.z.number(),
        optimizedImages: zod_1.z.number(),
        missingAltText: zod_1.z.number(),
...
class OnPageSEOService {
     ...
     */
    async analyzeOnPageSEO(request) {
        const results = await Promise.all([
            this.analyzeTitleOptimization(request.title || '', request.targetKeywords || []),
            this.analyzeMetaDescription(request.metaDescription || '', request.targetKeywords || []),
            this.analyzeHeadingStructure(request.content, request.targetKeywords || []),
            this.analyzeContentOptimization(request.content, request.targetKeywords || []),
            this.analyzeImageOptimization(request.images || []),
            this.analyzeLinkStructure(request.links || [], request.content)
        ]);
        const [titleAnalysis, metaAnalysis, headingAnalysis, contentAnalysis, imageAnalysis, linkAnalysis] = results;
        ...
        return {
            ...
            technical: {
                pageSpeed: {
                    desktop: { score: 0, loadTime: 0, firstContentfulPaint: 0, largestContentfulPaint: 0, timeToInteractive: 0 },
                    mobile: { score: 0, loadTime: 0, firstContentfulPaint: 0, largestContentfulPaint: 0, timeToInteractive: 0 },
                    coreWebVitals: { lcp: 0, fid: 0, cls: 0, passed: false },
                    suggestions: []
                },
                mobile: { responsive: true, mobileOptimized: true, touchElementsSize: true, viewportConfigured: true, score: 85 },
                schema: { present: [], missing: [], errors: [], suggestions: [], score: 0 },
                canonicalization: { hasCanonical: false, selfReferencing: false, issues: [], score: 50 },
                indexability: {
                    indexable: true,
                    robotsTxt: { exists: true, accessible: true, blocks: false, errors: [] },
     ...
     */
    async analyzeTitleOptimization(title, targetKeywords) {
        if (!title.trim()) {
            return {
                text: '',
                length: 0,
...
Provide specific suggestions for improvement and 3 optimized alternatives.`;
     ...
     */
    async analyzeMetaDescription(metaDescription, targetKeywords) {
        if (!metaDescription.trim()) {
            return {
                ...
                callToAction: false,
                uniqueness: 0,
                suggestions: ['Add a compelling meta description (150-160 characters) with target keywords and a call-to-action'],
                score: 0
            };
        }
...
Provide specific improvement suggestions.`;
        try {
            const result = await (0, ai_1.generateObject)({
                ...
                schema: zod_1.z.object({
                    analysis: zod_1.z.object({
                        keywordPresence: zod_1.z.boolean(),
                        callToAction: zod_1.z.boolean(),
                        lengthOptimal: zod_1.z.boolean(),
                        valueProposition: zod_1.z.boolean(),
                        uniqueness: zod_1.z.number().min(0).max(100),
            ...
            return {
                ...
                suggestions: analysis.suggestions,
                score: this.calculateMetaDescriptionScore(metaDescription, targetKeywords, analysis)
            };
        }
        catch (error) {
            // Fallback analysis
     ...
     */
    async analyzeHeadingStructure(content, targetKeywords) {
        ...
        if (headings.length === 0) {
            return {
                ...
                hierarchy: false,
                suggestions: ['Add proper heading structure (H1, H2, H3) to organize content'],
                score: 0
            };
        }
...
Provide optimization suggestions.`;
     ...
     */
    async analyzeContentOptimization(content, targetKeywords) {
...
Provide specific optimization recommendations.`;
        try {
            ...
            const keywordDensity = result.object.keywordAnalysis.map(ka => ({
                keyword: ka.keyword,
                count: ka.count,
                density: ka.density,
            ...
            return {
                ...
                topicCoverage,
                score: this.calculateContentScore(wordCount, readability, result.object.topicCoverage.coverage)
            };
        }
        catch (error) {
            // Fallback analysis
     ...
     */
    async analyzeImageOptimization(images) {
        if (images.length === 0) {
            return {
                totalImages: 0,
                optimizedImages: 0,
...
Provide specific improvement suggestions.`;
     ...
     */
    async analyzeLinkStructure(links, content) {
...
Provide specific link building suggestions.`;
        try {
            ...
            const internalSuggestions = result.object.internal.suggestions.map(s => ({
                anchor: s.anchor,
            ...
            const externalSuggestions = result.object.external.suggestions.map(s => ({
                anchor: s.anchor,
            ...
            return {
                ...
                score: (result.object.internal.quality + result.object.external.quality) / 2
            };
        }
        catch (error) {
            // Fallback analysis
     ...
     */
    async generateRecommendations(request, analyses) {
        ...
        if (analyses.title.score < 80) {
            recommendations.push({
                ...
                type: seo_engine_1.SEORecommendationType.TITLE_OPTIMIZATION,
                priority: 'high',
                category: 'meta',
                title: 'Optimize Page Title',
                description: 'Your title needs optimization for better SEO performance',
                currentValue: analyses.title.text,
                suggestedValue: analyses.title.suggestions[0] || 'Create compelling, keyword-optimized title',
                impact: 85,
                effort: 'easy',
                timeframe: '15 minutes',
                implementation: 'Update the title tag to include target keywords naturally within 30-60 characters',
        ...
        if (analyses.meta.score < 70) {
            recommendations.push({
                id: `rec_${recommendationId++}`,
                type: seo_engine_1.SEORecommendationType.META_DESCRIPTION,
                priority: 'high',
                category: 'meta',
                ...
                implementation: 'Write a 150-160 character meta description that includes target keywords and encourages clicks',
        ...
        if (analyses.content.wordCount < 300) {
            recommendations.push({
                id: `rec_${recommendationId++}`,
                type: seo_engine_1.SEORecommendationType.CONTENT_LENGTH,
                priority: 'critical',
                category: 'content',
                ...
                effort: 'difficult',
                timeframe: '2-4 hours',
                implementation: 'Expand content with valuable information, examples, and detailed explanations',
        ...
        if (analyses.headings.score < 70) {
            recommendations.push({
                id: `rec_${recommendationId++}`,
                type: seo_engine_1.SEORecommendationType.HEADING_STRUCTURE,
                priority: 'medium',
                category: 'content',
                title: 'Improve Heading Structure',
                description: 'Heading hierarchy needs optimization for better content organization',
                impact: 60,
                effort: 'moderate',
                timeframe: '30 minutes',
                implementation: 'Organize content with proper H1, H2, H3 hierarchy and include keywords naturally',
        ...
        if (analyses.images.missingAltText > 0) {
            recommendations.push({
                id: `rec_${recommendationId++}`,
                type: seo_engine_1.SEORecommendationType.IMAGE_ALT_TEXT,
                priority: 'medium',
                category: 'images',
                ...
                implementation: 'Add descriptive, keyword-relevant alt text to all images',
        ...
        if (analyses.links.internal.totalLinks < 3) {
            recommendations.push({
                id: `rec_${recommendationId++}`,
                type: seo_engine_1.SEORecommendationType.INTERNAL_LINKING,
                priority: 'medium',
                category: 'links',
                title: 'Add Internal Links',
                description: 'Content needs more internal links to improve SEO and user experience',
                ...
                implementation: 'Add contextual internal links to related content using descriptive anchor text',
     ...
     */
    basicTitleAnalysis(title, targetKeywords) {
        const keywordPresence = targetKeywords.some(keyword => title.toLowerCase().includes(keyword.toLowerCase()));
        const keywordPosition = keywordPresence ?
            Math.min(...targetKeywords.map(k => title.toLowerCase().indexOf(k.toLowerCase())).filter(i => i >= 0)) : -1;
        const lengthScore = title.length >= 30 && title.length <= 60 ? 100 :
            title.length > 60 ? 60 : 40;
        return {
            text: title,
            length: title.length,
            keywordPresence,
    ...
    basicMetaDescriptionAnalysis(metaDescription, targetKeywords) {
        const keywordPresence = targetKeywords.some(keyword => metaDescription.toLowerCase().includes(keyword.toLowerCase()));
        const callToAction = /\b(learn|discover|find|get|try|buy|shop|download|sign up|subscribe)\b/i.test(metaDescription);
        const lengthOptimal = metaDescription.length >= 150 && metaDescription.length <= 160;
        return {
            text: metaDescription,
            length: metaDescription.length,
            keywordPresence,
            ...
            score: this.calculateMetaDescriptionScore(metaDescription, targetKeywords, {
                keywordPresence,
                callToAction,
                lengthOptimal,
                uniqueness: 80
    ...
    basicHeadingAnalysis(headings, targetKeywords) {
        const h1Count = headings.filter(h => h.level === 1).length;
        const h1Text = headings.find(h => h.level === 1)?.text;
        const keywordOptimization = headings.reduce((score, heading) => {
            const hasKeyword = targetKeywords.some(keyword => heading.text.toLowerCase().includes(keyword.toLowerCase()));
        ...
        return {
            structure: headings.map((h, i) => ({
                level: h.level,
                text: h.text,
                keywordPresence: targetKeywords.some(k => h.text.toLowerCase().includes(k.toLowerCase())),
                position: i,
            ...
            score: this.calculateHeadingScore(headings, { keywordOptimization, properHierarchy: hierarchy, h1Count })
        };
    }
    basicContentAnalysis(content, targetKeywords) {
        const wordCount = this.countWords(content);
        const readability = this.calculateBasicReadability(content);
    ...
    basicImageAnalysis(images) {
        ...
        return {
            ...
            details: images.map(img => ({
                ...
                suggestions: [
                    ...((!img.alt || img.alt.trim() === '') ? ['Add descriptive alt text'] : [])
                ]
            })),
            score: images.length === 0 ? 100 : (optimizedImages / images.length) * 100
        };
    }
    basicLinkAnalysis(links) {
        const internalLinks = links.filter(l => l.internal);
        const externalLinks = links.filter(l => !l.internal);
        return {
            internal: {
            ...
            external: {
                ...
                uniqueDomains: new Set(externalLinks.map(l => new URL(l.href).hostname)).size,
                authorityScore: 70,
                brokenLinks: 0,
                noFollowRatio: 0.3,
                suggestions: [],
            ...
            score: (internalLinks.length >= 3 ? 80 : 50 + externalLinks.length > 0 ? 75 : 60) / 2
        };
    }
    /**
     * Utility methods
    ...
    calculateBasicReadability(content) {
        ...
        // Gunning Fog Index
        const complexWords = content.split(/\s+/).filter(word => this.countSyllablesInWord(word) >= 3).length;
        const fog = 0.4 * (avgWordsPerSentence + (100 * complexWords / words));
        // Coleman-Liau Index
        const avgCharsPerWord = content.replace(/\s+/g, '').length / words;
        const coleman = (0.0588 * (avgCharsPerWord * 100 / words)) - (0.296 * (sentences * 100 / words)) - 15.8;
        // ARI
    ...
    calculateTitleScore(title, targetKeywords, analysis) {
        let score = 0;
        // Keyword presence (40 points)
        if (analysis.keywordPresence)
            score += 40;
    ...
    calculateMetaDescriptionScore(metaDescription, targetKeywords, analysis) {
        let score = 0;
        // Keyword presence (35 points)
        if (analysis.keywordPresence)
            score += 35;
        // Length optimization (25 points)
        if (analysis.lengthOptimal || (metaDescription.length >= 150 && metaDescription.length <= 160))
            score += 25;
        // Call to action (20 points)
        if (analysis.callToAction)
            score += 20;
        // Uniqueness (20 points)
        score += (analysis.uniqueness || 80) * 0.20;
        return Math.min(100, score);
    }
    calculateHeadingScore(headings, analysis) {
        let score = 0;
        ...
        if (analysis.h1Count === 1)
            ...
            score += 10; // Multiple H1s is suboptimal
        // Hierarchy (25 points)
    ...
    calculateContentScore(wordCount, readability, topicCoverage) {
        let score = 0;
...
exports.OnPageSEOService = OnPageSEOService;
