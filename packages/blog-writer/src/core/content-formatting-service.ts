...
export interface ContentModification {
  type: 'length_reduction' | 'format_change' | 'structure_change' | 'media_optimization' | 'seo_optimization';
  description: string;
...
export class ContentFormattingService {
  ...
  public async formatForPlatform(
    ...
    formattedContent = await this.applyFormatConversion(
      formattedContent, capabilities, mergedOptions, modifications
    );
    
    // Apply length constraints
    formattedContent = await this.applyLengthConstraints(
      formattedContent, capabilities, mergedOptions, modifications, warnings
    );
    
    // Apply structure adaptations
    formattedContent = await this.applyStructureAdaptations(
      formattedContent, platformName, capabilities, mergedOptions, modifications
    );
    
    // Optimize media
    formattedContent = await this.optimizeMedia(
      formattedContent, capabilities, modifications, warnings
    );
    
    // Apply SEO optimizations
    if (this.config.optimizeForSEO) {
      formattedContent = await this.applySEOOptimizations(
        formattedContent, platformName, capabilities, modifications
      );
    }
    
    // Apply platform-specific enhancements
    formattedContent = await this.applyPlatformSpecificEnhancements(
      formattedContent, platformName, mergedOptions, modifications
    );
    
    // Generate suggestions
    suggestions.push(...this.generateSuggestions(formattedContent, platformName, capabilities));
    
    // Calculate adaptation score
    const adaptationScore = this.calculateAdaptationScore(
      content, formattedContent, modifications
    );
  ...
  public async formatForMultiplePlatforms(
    content: BlogPost,
    platforms: Array<{ name: string; capabilities: PlatformCapabilities; options?: FormatOptions }>
  ...
  private async applyFormatConversion(
    ...
    if (content.format !== targetFormat) {
      ...
      content.content = await this.convertFormat(content.content, content.format, targetFormat);
  ...
  private async applyLengthConstraints(
    ...
    if (capabilities.maxTitleLength && content.title.length > capabilities.maxTitleLength) {
      const originalTitle = content.title;
      content.title = this.truncateTitle(content.title, capabilities.maxTitleLength);
    ...
    if (capabilities.maxDescriptionLength && content.excerpt && 
        content.excerpt.length > capabilities.maxDescriptionLength) {
      const originalExcerpt = content.excerpt;
      content.excerpt = this.truncateDescription(content.excerpt, capabilities.maxDescriptionLength);
  ...
  private async applyStructureAdaptations(
    ...
    switch (platformName.toLowerCase()) {
      ...
      default:
        // Generic adaptations
        content = this.applyGenericAdaptations(content, capabilities, modifications);
        break;
    }
  ...
  private async optimizeMedia(
    ...
    if (content.featuredImage) {
      if (!capabilities.supportsImages) {
        warnings.push('Platform does not support images. Featured image will be removed.');
        ...
        if (capabilities.maxImageSize && content.featuredImage.size > capabilities.maxImageSize) {
          warnings.push(`Featured image exceeds maximum size of ${capabilities.maxImageSize} bytes`);
        }
        
        // Check supported formats
        if (capabilities.supportedImageFormats) {
          const extension = this.getFileExtension(content.featuredImage.filename);
    ...
    if (content.gallery && content.gallery.length > 0) {
      if (!capabilities.supportsGalleries) {
        warnings.push('Platform does not support image galleries. Gallery will be removed.');
  ...
  private async applySEOOptimizations(
    ...
    if (capabilities.supportsCustomMeta) {
      ...
      if (content.seo.metaDescription && content.seo.metaDescription.length > 160) {
        ...
        content.seo.metaDescription = content.seo.metaDescription.substring(0, 157) + '...';
  ...
  private async applyPlatformSpecificEnhancements(
    ...
    if (platformRule?.rules.optimizeForEngagement) {
      content = this.optimizeForEngagement(content, platformName, modifications);
    }
    
    // Apply custom transformations
    if (platformRule?.rules.customTransformations) {
      for (const transform of platformRule.rules.customTransformations) {
        content = this.applyCustomTransformation(content, transform, modifications);
      }
    }
  ...
  private adaptForLinkedIn(
    ...
    if (!content.content.includes('professional') && !content.content.includes('business')) {
  ...
  private adaptForWordPress(
    ...
    if (content.seo.focusKeyword) {
      ...
      if (firstPara && !firstPara[1].toLowerCase().includes(content.seo.focusKeyword.toLowerCase())) {
  ...
  private applyGenericAdaptations(
    ...
    if (!capabilities.supportsCustomCSS) {
      content.content = content.content.replace(/<style[^>]*>.*?<\/style>/gis, '');
  ...
  private mergeOptions(options?: FormatOptions, ruleOptions?: any): FormatOptions {
    return {
      ...ruleOptions,
      ...options
    };
  ...
  private determineTargetFormat(
    ...
    } else if (capabilities.supportedFormats.includes(ContentFormat.RICH_TEXT)) {
  ...
  private mapContentStatus(status: string): 'draft' | 'published' | 'private' | 'unlisted' {
  ...
  private generateCallToAction(platformName: string): string {
    const ctas = {
      linkedin: 'What are your thoughts? Share your experience in the comments below.',
      twitter: 'What do you think? Reply with your thoughts!',
      medium: 'If you found this helpful, please give it a clap and follow for more insights.',
    ...
    switch (transform.type) {
      case 'replace':
        if (transform.pattern && transform.replacement !== undefined) {
          content.content = content.content.replace(transform.pattern, transform.replacement);
        }
        break;
      case 'append':
  ...
  private calculateAdaptationScore(
    ...
    const lengthRatio = formatted.adaptedWordCount / formatted.originalWordCount;
    if (lengthRatio < 0.7) {
  ...
  private generateSuggestions(
    ...
    if (platformName === 'linkedin' && content.adaptedWordCount > 1000) {
      suggestions.push('Consider breaking this into multiple LinkedIn posts for better engagement');
    }
    
    if (platformName === 'twitter' && content.adaptedWordCount > 100) {
      suggestions.push('This content might work better as a Twitter thread or blog link');
    }
    ...
    if (!content.metadata.keywords || content.metadata.keywords.length === 0) {
      suggestions.push('Adding relevant keywords could improve discoverability');
    }
  ...
  private generateValidationSuggestions(
    ...
    if (capabilities.supportsTags && (!content.metadata.keywords || content.metadata.keywords.length === 0)) {
      suggestions.push({
        field: 'keywords',
  ...
  public getPlatformRule(platformName: string): ContentFormattingRule | undefined {
