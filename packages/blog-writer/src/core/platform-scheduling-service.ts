...
export class PlatformSchedulingService {
  ...
  public async createSchedule(
    ...
    console.log(
      `Created schedule: ${schedule.name} for ${new Date(schedule.scheduledTime).toISOString()}`,
    );
  ...
  public async cancelSchedule(scheduleId: string): Promise<boolean> {
    ...
    if (this.prisma) {
      await this.updateScheduleStatusInDatabase(
        scheduleId,
  ...
  public getQueueStatistics(queueId: string): QueueStatistics | null {
    ...
    const completedItemsWithTiming = items.filter(
      i => i.status === QueueItemStatus.COMPLETED && i.updatedAt,
    );

    const averageProcessingTime =
      completedItemsWithTiming.length > 0
        ? completedItemsWithTiming.reduce((acc, item) => {
            return (
              acc + (item.updatedAt.getTime() - item.createdAt.getTime()) / 1000
            );
    ...
    const lastProcessedItem = completedItemsWithTiming.sort(
  ...
  private convertFormattedContentToBlogPost(
  ...
  private calculateNextExecution(
    ...
    switch (pattern.type) {
      ...
      case 'weekly':
        base.setDate(
          base.getDate() + pattern.interval * 7 * (executionCount + 1),
        );
        break;
      case 'monthly':
        base.setMonth(
          base.getMonth() + pattern.interval * (executionCount + 1),
        );
        break;
      case 'custom':
  ...
  private sortQueueItemsArray(
    items: QueueItem[],
    order: QueueProcessingOrder,
  ): void {
    switch (order) {
  ...
  public getScheduleStatistics(): ScheduleStatistics {
    ...
    const failedSchedules = schedules.filter(
      s => s.status === ScheduleStatus.FAILED,
    ).length;
    ...
    const executionsToday = schedules.reduce((count, schedule) => {
      return (
        count +
        schedule.executionHistory.filter(
          exec => exec.executedAt >= today && exec.executedAt < tomorrow,
        ).length
      );
    }, 0);
    ...
    const totalExecutions = schedules.reduce(
      (count, schedule) => count + schedule.executionHistory.length,
      0,
    );
    const successfulExecutions = schedules.reduce(
      (count, schedule) =>
        count + schedule.executionHistory.filter(exec => exec.success).length,
      0,
    );
  ...
  private async updateScheduleExecutionInDatabase(
    ...
    try {
      console.log(
        `Storing execution ${execution.id} for schedule ${scheduleId}`,
      );
    } catch (error) {
