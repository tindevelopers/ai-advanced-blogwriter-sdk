...
const ImageOptimizationSchema = z.object({
  analysis: z.array(z.object({
    ...
    issues: z.array(z.object({
      type: z.enum(['missing_alt', 'poor_alt', 'oversized', 'wrong_format', 'not_optimized']),
...
export class OnPageSEOService {
   ...
   */
  async analyzeOnPageSEO(request: OnPageAnalysisRequest): Promise<OnPageSEOAnalysis> {
   ...
   */
  private async analyzeTitleOptimization(title: string, targetKeywords: string[]): Promise<TitleAnalysis> {
    if (!title.trim()) {
      return {
        text: '',
        length: 0,
...
Provide specific suggestions for improvement and 3 optimized alternatives.`;
   ...
   */
  private async analyzeMetaDescription(metaDescription: string, targetKeywords: string[]): Promise<MetaDescriptionAnalysis> {
...
Provide specific improvement suggestions.`;
   ...
   */
  private async analyzeHeadingStructure(content: string, targetKeywords: string[]): Promise<HeadingAnalysis> {
...
Provide optimization suggestions.`;
   ...
   */
  private async analyzeContentOptimization(content: string, targetKeywords: string[]): Promise<ContentAnalysis> {
...
Provide specific optimization recommendations.`;
    ...
    try {
      ...
      const keywordDensity: KeywordDensityAnalysis[] = result.object.keywordAnalysis.map(ka => ({
        keyword: ka.keyword,
        count: ka.count,
        density: ka.density,
   ...
   */
  private async analyzeImageOptimization(images: Array<{ src: string; alt?: string; title?: string }>): Promise<ImageOptimization> {
    if (images.length === 0) {
      return {
        totalImages: 0,
        optimizedImages: 0,
...
Provide specific link building suggestions.`;
    ...
    try {
      ...
      const internalSuggestions: InternalLinkSuggestion[] = result.object.internal.suggestions.map(s => ({
        anchor: s.anchor,
      ...
      const externalSuggestions: ExternalLinkSuggestion[] = result.object.external.suggestions.map(s => ({
        anchor: s.anchor,
   ...
   */
  private basicTitleAnalysis(title: string, targetKeywords: string[]): TitleAnalysis {
  ...
  private basicMetaDescriptionAnalysis(metaDescription: string, targetKeywords: string[]): MetaDescriptionAnalysis {
    const keywordPresence = targetKeywords.some(keyword => 
  ...
  private basicHeadingAnalysis(headings: any[], targetKeywords: string[]): HeadingAnalysis {
  ...
  private basicContentAnalysis(content: string, targetKeywords: string[]): ContentAnalysis {
    const wordCount = this.countWords(content);
    const readability = this.calculateBasicReadability(content);
    
    const keywordDensity: KeywordDensityAnalysis[] = targetKeywords.map(keyword => {
  ...
  private basicImageAnalysis(images: Array<{ src: string; alt?: string; title?: string }>): ImageOptimization {
  ...
  private basicLinkAnalysis(links: Array<{ href: string; text: string; internal: boolean }>): LinkAnalysis {
    const internalLinks = links.filter(l => l.internal);
    const externalLinks = links.filter(l => !l.internal);
  ...
  private calculateTitleScore(title: string, targetKeywords: string[], analysis: any): number {
    let score = 0;
    ...
    if (analysis.lengthOptimal || (title.length >= 30 && title.length <= 60)) score += 30;
  ...
  private calculateMetaDescriptionScore(metaDescription: string, targetKeywords: string[], analysis: any): number {
    let score = 0;
    ...
    if (analysis.lengthOptimal || (metaDescription.length >= 150 && metaDescription.length <= 160)) score += 25;
  ...
  private calculateContentScore(wordCount: number, readability: any, topicCoverage: number): number {
    let score = 0;
