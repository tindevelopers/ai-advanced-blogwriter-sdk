...
export class EditorialCalendarService {
  ...
  async createCalendar(
    ...
    try {
      ...
      throw new Error(
        `Calendar creation failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
  ...
  async generateCalendar(
    request: EditorialCalendarRequest,
  ): Promise<EditorialCalendarResponse> {
    ...
    try {
      ...
      const entries = await Promise.all(
        mockResult.object.entries.map(entryData =>
      ...
      throw new Error(
        `Calendar generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
   ...
   */
  async addEntry(
    entryData: Partial<EditorialCalendarEntry>,
    ...
    try {
      ...
      throw new Error(
        `Calendar entry creation failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
  ...
  async updateEntry(
    entryId: string,
    updates: Partial<EditorialCalendarEntry>,
  ): Promise<EditorialCalendarEntry> {
    if (!this.prisma) {
      throw new Error('Database connection required for calendar operations');
    }
    ...
    try {
      ...
      throw new Error(
        `Calendar entry update failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
  ...
  async addMilestone(
    entryId: string,
    milestone: Partial<CalendarMilestone>,
    ...
    try {
      ...
      throw new Error(
        `Milestone creation failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
  ...
  async trackTime(
    ...
    try {
      ...
      throw new Error(
        `Time tracking failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
  ...
  async getCalendarEntries(
    ...
    try {
      ...
      throw new Error(
        `Calendar retrieval failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
    ...
    try {
      ...
      return {
        ...
        milestones: milestones.map(milestone =>
          this.mapPrismaMilestoneToType(milestone),
        ),
      };
    } catch (error) {
      console.error('Error getting deadlines:', error);
      throw new Error(
        `Deadline retrieval failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
  ...
  async getCalendarAnalytics(
    ...
    try {
      ...
      const onTimeEntries = entries.filter(
        e =>
          e.status === 'published' &&
          e.publishDate &&
          e.dueDate &&
      ...
      const statusCounts = entries.reduce(
        (acc, entry) => {
          acc[entry.status as CalendarEntryStatus] =
            (acc[entry.status as CalendarEntryStatus] || 0) + 1;
          return acc;
        },
        {} as Record<CalendarEntryStatus, number>,
      );

      const priorityCounts = entries.reduce(
        (acc, entry) => {
          acc[entry.priority as Priority] =
            (acc[entry.priority as Priority] || 0) + 1;
          return acc;
        },
        {} as Record<Priority, number>,
      );

      const assignmentCounts = entries.reduce(
        (acc, entry) => {
          if (entry.assignedTo) {
      ...
      const completedWithTime = entries.filter(
        e => e.status === 'published' && e.createdAt && e.publishDate,
      );
      const avgCompletionTime =
        completedWithTime.length > 0
          ? completedWithTime.reduce(
              (sum, e) =>
                sum + (e.publishDate!.getTime() - e.createdAt.getTime()),
              0,
            ) /
      ...
      const analytics: CalendarAnalytics = {
        productivity: {
          ...
          onTimeDelivery:
            completedEntries > 0 ? onTimeEntries / completedEntries : 0,
        },
        workload: {
          totalEntries,
          byStatus: statusCounts,
          byPriority: priorityCounts,
          upcomingDeadlines: entries.filter(
            e =>
              e.dueDate &&
              e.dueDate > new Date() &&
  ...
  private buildCalendarGenerationPrompt(
    request: EditorialCalendarRequest,
  ): string {
    ...
    const contentTypes =
      request.contentTypes?.join(', ') || 'Blog posts, articles';
  ...
  private async createDefaultMilestones(
    ...
    const publishDate = entryData.publishDate
  ...
  private async updateMilestonesForStatus(
    entryId: string,
    status: CalendarEntryStatus,
  ): Promise<void> {
    if (!this.prisma) return;
  ...
  private generateCalendarSummary(
  ...
  ): EditorialCalendarResponse['summary'] {
    const totalEntries = entries.length;
    ...
    const byPriority = entries.reduce(
      (acc, entry) => {
    ...
    const now = new Date();
    const upcomingDeadlines = entries.filter(
      entry =>
        entry.dueDate &&
        entry.dueDate > now &&
  ...
  private mapPrismaCalendarToType(prismaCalendar: any): EditorialCalendar {
    return {
      ...
      entries:
        prismaCalendar.entries?.map(entry =>
          this.mapPrismaEntryToType(entry),
        ) || [],
    };
  }
  ...
  private mapPrismaEntryToType(prismaEntry: any): EditorialCalendarEntry {
    return {
      ...
      milestones:
        prismaEntry.milestones?.map(m => this.mapPrismaMilestoneToType(m)) ||
        [],
      timeTracking:
        prismaEntry.timeTracking?.map(t => this.mapPrismaTimeEntryToType(t)) ||
        [],
