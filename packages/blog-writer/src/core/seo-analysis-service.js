...
exports.SEOAnalysisService = void 0;
const dataforseo_service_1 = require("./dataforseo-service");
const keyword_research_service_1 = require("./keyword-research-service");
const onpage_seo_service_1 = require("./onpage-seo-service");
const meta_schema_service_1 = require("./meta-schema-service");
const readability_scoring_service_1 = require("./readability-scoring-service");
...
class SEOAnalysisService {
    constructor(config) {
        ...
        this.keywordResearchService = new keyword_research_service_1.KeywordResearchService({
            model: config.model,
            prisma: config.prisma,
        ...
        this.onPageSEOService = new onpage_seo_service_1.OnPageSEOService({
            model: config.model,
            prisma: config.prisma,
            cacheResults: config.cacheResults,
            cacheTTL: config.cacheTTL
        });
        this.metaSchemaService = new meta_schema_service_1.MetaSchemaService({
            model: config.model,
            prisma: config.prisma,
        ...
        this.readabilityScoringService = new readability_scoring_service_1.ReadabilityScoringService({
            model: config.model,
            prisma: config.prisma,
            cacheResults: config.cacheResults,
            cacheTTL: config.cacheTTL
     ...
     */
    async analyzeSEO(request, options = {}, callbacks) {
        ...
        try {
            ...
            if (analysisOptions.includeCompetitorAnalysis && request.competitorUrls && this.dataForSEOService) {
                ...
                try {
                    const competitorResponse = await this.dataForSEOService.analyzeCompetitors(request.targetKeywords?.[0] || content.title, request.competitorUrls);
            ...
            const recommendations = await this.generateComprehensiveRecommendations(results, analysisOptions);
            ...
            const qualityGateResults = this.applyQualityGates(results);
     ...
     */
    async quickSEOHealthCheck(blogPostId) {
        try {
     ...
     */
    async generateSEOImprovementRoadmap(analysisResult) {
        const recommendations = analysisResult.recommendations;
        const quickWins = recommendations.filter(r => r.effort === 'easy' && r.impact >= 70).slice(0, 5);
        const shortTerm = recommendations.filter(r => r.effort === 'moderate' && r.impact >= 60).slice(0, 8);
        const longTerm = recommendations.filter(r => r.effort === 'difficult' || (r.effort === 'moderate' && r.impact < 60)).slice(0, 10);
     ...
     */
    async getContentFromBlogPost(blogPostId) {
        if (!this.config.prisma) {
    ...
    calculateOverallScore(categoryScores, contentQuality) {
        const weights = {
            onPage: 0.25,
            technical: 0.20,
            content: 0.25,
    ...
    async generateComprehensiveRecommendations(result, options) {
        const recommendations = [];
        let recId = 1;
        ...
        if (result.contentQuality.recommendations) {
            recommendations.push(...result.contentQuality.recommendations.map(r => ({
                id: `rec_${recId++}`,
                type: seo_engine_1.SEORecommendationType.CONTENT_LENGTH, // Map based on category
                priority: r.priority,
                category: r.category,
                title: r.title,
        ...
        if (result.keywordAnalysis.length === 0) {
            recommendations.push({
                id: `rec_${recId++}`,
                type: seo_engine_1.SEORecommendationType.KEYWORD_OPTIMIZATION,
                priority: 'high',
                category: 'keywords',
        ...
        if (result.readabilityScore.suggestions) {
            recommendations.push(...result.readabilityScore.suggestions.map(s => ({
                id: `rec_${recId++}`,
                type: seo_engine_1.SEORecommendationType.READABILITY,
...
exports.SEOAnalysisService = SEOAnalysisService;
