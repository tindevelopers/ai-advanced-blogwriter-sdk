...
export class ShopifyAdapter extends BasePlatformAdapter {
  ...
  private async authenticateWithAccessToken(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return {
          success: false,
          error:
            errorData.errors || `Authentication failed: ${response.statusText}`,
        };
      }
  ...
  private async getProductByHandle(
    handle: string,
  ): Promise<ShopifyProduct | null> {
    try {
      const response = await fetch(
        `${this.apiUrl}/products.json?handle=${handle}`,
        {
          headers: {
            'X-Shopify-Access-Token': this.accessToken!,
            'Content-Type': 'application/json',
  ...
  protected async publishContentInternal(
    ...
    try {
      ...
      const response = await fetch(
        `${this.apiUrl}/blogs/${this.blogId}/articles.json`,
        {
          method: 'POST',
          headers: {
            'X-Shopify-Access-Token': this.accessToken!,
      ...
      if (!response.ok) {
        ...
        return {
          success: false,
          error:
            errorData.errors || `Publishing failed: ${response.statusText}`,
        };
      }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return {
          ...
          error:
            errorData.errors || `Scheduling failed: ${response.statusText}`,
        };
      }
  ...
  public async update(
    ...
    try {
      ...
      const response = await fetch(
        `${this.apiUrl}/blogs/${this.blogId}/articles/${externalId}.json`,
        {
          method: 'PUT',
          headers: {
            'X-Shopify-Access-Token': this.accessToken!,
  ...
  protected async getAnalyticsInternal(
    ...
    try {
      ...
      const response = await fetch(
        `${this.apiUrl}/blogs/${this.blogId}/articles.json?limit=250`,
        {
          headers: {
            'X-Shopify-Access-Token': this.accessToken!,
            'Content-Type': 'application/json',
      ...
      const filteredArticles = articles.filter((article: any) => {
        ...
        return (
          publishedAt >= timeRange.startDate && publishedAt <= timeRange.endDate
        );
      });
