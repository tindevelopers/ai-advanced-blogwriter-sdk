...
export class WebflowAdapter extends BasePlatformAdapter {
  ...
  private async authenticateWithApiToken(creds: WebflowCredentials): Promise<AuthenticationResult> {
    if (!creds.apiToken) {
      return {
        success: false,
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return {
          success: false,
          error: errorData.msg || `Authentication failed: ${response.statusText}`
        };
      }
  ...
  private async authenticateWithOAuth2(creds: WebflowCredentials): Promise<AuthenticationResult> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return {
          success: false,
          error: errorData.msg || `OAuth2 authentication failed: ${response.statusText}`
        };
      }
  ...
  private async initializeBlogCollection(): Promise<void> {
    try {
      ...
      if (blogCollection) {
        ...
        throw new Error('No suitable blog collection found. Please specify a collection ID.');
      }
    } catch (error) {
      console.warn('Failed to initialize blog collection:', error);
      throw new Error('Failed to initialize blog collection. Please check your site configuration.');
    }
  }
  ...
  private async getCollection(collectionId: string): Promise<WebflowCollection> {
  ...
  protected async formatContentInternal(content: BlogPost, options?: FormatOptions): Promise<FormattedContent> {
    ...
    if (content.format === 'markdown') {
      formattedContent = this.convertMarkdownToWebflowRichText(formattedContent);
  ...
  private async processImages(images: MediaFile[]): Promise<MediaFile[]> {
    ...
    for (const image of images) {
      try {
        ...
        if (!image.url.includes('webflow.com') && !image.url.includes('https://')) {
  ...
  private async uploadAsset(image: MediaFile): Promise<WebflowAsset | null> {
    try {
      ...
      const response = await fetch(`${this.apiUrl}/sites/${this.siteId}/assets`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${this.apiToken}`,
  ...
  protected async publishContentInternal(content: FormattedContent, options?: PublishOptions): Promise<PublishResult> {
    try {
      const webflowItem: WebflowCMSItem = {
        name: content.title,
        slug: content.platformSpecific?.webflowSlug || this.generateSlug(content.title),
      ...
      const response = await fetch(`${this.apiUrl}/collections/${this.collectionId}/items`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${this.apiToken}`,
          'Content-Type': 'application/json',
  ...
  public async update(externalId: string, content: FormattedContent, options?: PublishOptions): Promise<PublishResult> {
    try {
      ...
      const response = await fetch(`${this.apiUrl}/collections/${this.collectionId}/items/${externalId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${this.apiToken}`,
          'Content-Type': 'application/json',
  ...
  protected async getAnalyticsInternal(timeRange: DateRange, options?: AnalyticsOptions): Promise<PlatformAnalytics> {
    ...
    try {
      ...
      const response = await fetch(`${this.apiUrl}/collections/${this.collectionId}/items?limit=100`, {
        headers: {
          'Authorization': `Bearer ${this.apiToken}`,
          'Accept': 'application/json'
        }
      ...
      const filteredItems = items.filter((item: any) => {
        ...
        return publishDate >= timeRange.startDate && publishDate <= timeRange.endDate;
      });
  ...
  private mapWebflowFieldType(webflowType: string): 'text' | 'number' | 'boolean' | 'date' | 'select' | 'multiselect' {
    switch (webflowType) {
      case 'PlainText':
      case 'RichText':
      case 'Link':
