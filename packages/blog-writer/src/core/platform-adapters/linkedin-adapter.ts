...
export class LinkedInAdapter extends BasePlatformAdapter {
  ...
  protected async performAuthentication(credentials: PlatformCredentials): Promise<AuthenticationResult> {
    ...
    try {
      ...
      const profile = await this.makeRequest('GET', 'people/~:(id,localizedFirstName,localizedLastName,profilePicture(displayImage~:playableStreams))');
  ...
  protected async formatContentInternal(content: BlogPost, options?: FormatOptions): Promise<FormattedContent> {
    ...
    let formattedContent: FormattedContent = {
      title: content.title,
      content: this.formatContentForLinkedIn(content.content, isArticle, maxLength),
  ...
  private formatContentForLinkedIn(content: string, isArticle: boolean, maxLength: number): string {
    if (!isArticle) {
  ...
  private createLinkedInPost(content: BlogPost): string {
    ...
    if (insights.length > 0) {
      postContent += insights.slice(0, 3).map(insight => `• ${insight}`).join('\n');
      postContent += '\n\n';
    }
    ...
    if (content.keywords && content.keywords.length > 0) {
      postContent += '\n\n';
      postContent += content.keywords.slice(0, 3)
  ...
  private createPostContent(content: string, maxLength: number): string {
    ...
    let postContent = '';
    for (const sentence of sentences) {
      if (postContent.length + sentence.length + 2 > maxLength - 100) { // Leave room for CTA
        break;
      }
  ...
  private cleanHtmlForLinkedIn(html: string): string {
    ...
    const allowedTags = ['p', 'br', 'strong', 'b', 'em', 'i', 'u', 'a', 'h1', 'h2', 'h3', 'ul', 'ol', 'li'];
    
    // Remove unsupported tags but keep content
    let cleaned = html.replace(/<(?!\/?(?:p|br|strong|b|em|i|u|a|h[1-3]|ul|ol|li)\b)[^>]*>/gi, '');
  ...
  protected async publishContentInternal(content: FormattedContent, options?: PublishOptions): Promise<PublishResult> {
    if (!this.personUrn) {
      throw new Error('Person URN not available - authentication may have failed');
    }
    
    const isArticle = content.platformSpecific?.isArticle || this.contentType === 'article';
  ...
  private async publishArticle(content: FormattedContent, options?: PublishOptions): Promise<PublishResult> {
    const articleData = {
  ...
  private async publishPost(content: FormattedContent, options?: PublishOptions): Promise<PublishResult> {
    const postData = {
      author: this.personUrn,
  ...
  protected async validatePlatformSpecificConstraints(
    ...
    if (!isArticle && !this.hasEngagementElements(content.content)) {
      suggestions.push({
        field: 'content',
        suggestion: 'Consider adding a question or call-to-action to increase engagement',
        impact: 'medium'
      });
    }
  }
