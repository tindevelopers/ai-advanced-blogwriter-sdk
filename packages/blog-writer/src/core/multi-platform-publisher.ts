...
export class MultiPlatformPublisherService implements MultiPlatformPublisher {
  ...
  public async publishToSelected(
    ...
    if (this.prisma) {
      publishSessions = await this.createPublishSessions(
        content.id,
        validPlatforms,
        options,
      );
    ...
    if (options?.requireAllSuccess) {
      ...
      return this.publishAllOrNothing(
        content,
        orderedPlatforms,
        options,
        results,
  ...
  private async publishBestEffort(
    ...
    for (const batch of batches) {
      const batchPromises = batch.map(async platformName => {
        try {
          ...
          if (this.prisma && publishSessions.length > 0) {
            await this.updatePublishSession(
              publishSessions[0],
              platformName,
              result,
            );
      ...
      if (options?.platformSpecificOptions?.delayBetweenBatches) {
        await this.delay(
          options.platformSpecificOptions.delayBetweenBatches as number,
        );
      }
    }
  ...
  private async publishAllOrNothing(
    ...
    try {
      ...
      console.warn(
        `Rolling back published content due to failure on ${error.platform}`,
      );
  ...
  private async publishToPlatform(
    ...
    const platformOptions =
      options?.platformSpecificOptions?.[platformName] || options;
    ...
    const formattedContent = await adapter.formatContent(
      content,
      formatOptions,
    );
  ...
  public async getAggregatedAnalytics(
    ...
    for (const platformName of targetPlatforms) {
      ...
      try {
        ...
        console.warn(
          `Failed to get analytics for ${platformName}:`,
          error.message,
        );
      }
    }
  ...
  public async bulkPublish(
    ...
    for (const batch of batches) {
      const batchPromises = batch.map(async content => {
        try {
          const result = await this.publishToSelected(
            content,
            platforms,
            options,
          );
  ...
  public async checkPlatformHealth(): Promise<PlatformHealthReport> {
    ...
    recommendations.push(
      ...this.generateHealthRecommendations(issues, platformHealth),
    );
  ...
  private async rollbackPublishedContent(
    content: BlogPost,
    platforms: string[],
  ): Promise<void> {
    ...
    const averages = {
      avgEngagementRate:
        platformData.length > 0
          ? platformData.reduce((acc, a) => acc + a.engagementRate, 0) /
            platformData.length
          : 0,
      avgConversionRate:
        platformData.length > 0
          ? platformData.reduce((acc, a) => acc + a.conversionRate, 0) /
            platformData.length
          : 0,
      avgBounceRate:
        platformData.length > 0
          ? platformData.reduce((acc, a) => acc + a.bounceRate, 0) /
            platformData.length
          : 0,
    };
  ...
  private generateCrossPlatformInsights(
    ...
    if (platformData.length > 1) {
      const avgEngagement =
        platformData.reduce((acc, p) => acc + p.engagementRate, 0) /
        platformData.length;
  ...
  private generatePlatformRecommendations(
    ...
    platformData.forEach(platform => {
      if (platform.engagementRate < 0.02) {
        // Less than 2% engagement
        recommendations.push({
      ...
      if (platform.bounceRate > 0.7) {
        // High bounce rate
        recommendations.push({
          platform: platform.platformName,
          type: 'content',
          recommendation:
            'Improve content quality and relevance to reduce bounce rate',
  ...
  private generatePlatformComparisons(
    ...
    for (const metric of metrics) {
      ...
      const secondMax = Math.max(
        ...Object.values(platformMetrics).filter(v => v !== maxValue),
      );
      const difference =
        secondMax > 0 ? ((maxValue - secondMax) / secondMax) * 100 : 0;
  ...
  private generatePlatformRankings(
    ...
    for (const metric of metrics) {
      const platformValues = platformData
        .map(p => ({
  ...
  private determineWinnerByMetric(
    platformData: PlatformAnalytics[],
