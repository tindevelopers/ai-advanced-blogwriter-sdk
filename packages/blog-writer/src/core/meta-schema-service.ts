...
const MetaTagGenerationSchema = z.object({
  metaTags: z.object({
    title: z.string(),
...
export class MetaSchemaService {
  private config: MetaSchemaConfig;

  constructor(config: MetaSchemaConfig) {
    this.config = {
      cacheResults: true,
...
Focus on Article, FAQ, HowTo, Breadcrumb, Organization, and WebSite schemas.`;
  ...
  validateSchemaMarkup(schema: any): { valid: boolean; errors: string[] } {
    ...
    try {
      ...
      if (schema['@type'] === 'Article' || schema['@type'] === 'BlogPosting') {
        ...
        if (!schema.image) errors.push('Article missing image');
      }
   ...
   */
  generateMetaTagHTML(metaTags: MetaTagSuggestions): string {
    const html: string[] = [];
   ...
   */
  generateSchemaHTML(schema: SchemaMarkup): string {
