...
export class ContentRouter {
  ...
  async routeContent(options: {
    ...
    const baseConfig = contentTypeDetector.getRoutingConfig(contentTypeResult.contentType);
    ...
    const alternatives = await this.generateAlternatives(routingConfig, options);
    ...
    return {
      ...
      reasoning: this.generateReasoning(contentTypeResult, routingConfig, options),
  ...
  async routeForGoals(options: {
    ...
    targetAudience: 'beginners' | 'intermediate' | 'experts' | 'decision-makers' | 'general';
    industry?: string;
    ...
    const goalBasedStrategies: Record<string, Partial<ContentRoutingConfig>> = {
      'lead-generation': {
        ...
        requiredSections: ['introduction', 'value-proposition', 'call-to-action', 'social-proof'],
      },
      'brand-awareness': {
      ...
      'thought-leadership': {
        ...
        seoOptimization: 'advanced',
        qualityLevel: 'expert',
        requiredSections: ['expert-analysis', 'industry-trends', 'future-predictions', 'actionable-insights'],
      },
      'education': {
        promptStrategy: 'structured',
        seoOptimization: 'basic',
        qualityLevel: 'standard',
        requiredSections: ['learning-objectives', 'step-by-step-content', 'examples', 'summary'],
      },
      'product-promotion': {
        promptStrategy: 'conversational',
        seoOptimization: 'comprehensive',
        qualityLevel: 'premium',
        requiredSections: ['problem-identification', 'solution-presentation', 'benefits', 'social-proof', 'call-to-action'],
      },
      'seo-ranking': {
        promptStrategy: 'structured',
        seoOptimization: 'comprehensive',
        qualityLevel: 'premium',
        requiredSections: ['keyword-rich-introduction', 'comprehensive-coverage', 'internal-links', 'related-topics'],
      },
    };
  ...
  async createRoutingPipeline(
    ...
    for (const topicConfig of topics) {
      ...
      const priorityScore = this.calculatePriorityScore(topicConfig, routing, globalConfig);

      // Detect potential content series
      const contentSeries = this.detectContentSeries(topicConfig.topic, results);
  ...
  private async createRoutingConfig(
    ...
    return {
      ...
      wordCountTarget: {
        ...
        optimal: Math.floor((baseConfig.wordCountRange.min + baseConfig.wordCountRange.max) / 2),
      },
      requiredSections: [
      ...
      if (type !== primaryConfig.contentType) {
        ...
        alternatives.push({
          ...
          wordCountTarget: {
            ...
            optimal: Math.floor((altConfig.wordCountRange.min + altConfig.wordCountRange.max) / 2),
          },
  ...
  private createOptimizations(config: ContentRoutingConfig, options: any): string[] {
    const optimizations: string[] = [];

    if (options.keywords?.length > 0) {
      optimizations.push(`Optimize for keywords: ${options.keywords.slice(0, 3).join(', ')}`);
    }

    if (config.seoOptimization === 'basic') {
      optimizations.push('Consider upgrading to advanced SEO optimization for better rankings');
    }

    if (config.qualityLevel === 'standard' && options.businessContext) {
      optimizations.push('Consider premium quality level for business-critical content');
    }

    if (config.wordCountTarget.optimal < 1000) {
      optimizations.push('Consider increasing word count for better SEO performance');
    }

    return optimizations;
  }

  private generateReasoning(contentTypeResult: any, config: ContentRoutingConfig, options: any): string {
    const reasons = [];

    reasons.push(`Content type detected as ${config.contentType} with ${Math.round(contentTypeResult.confidence * 100)}% confidence`);
    reasons.push(`Selected ${config.template} template based on content structure requirements`);
    reasons.push(`Chose ${config.promptStrategy} prompt strategy for optimal ${options.audience || 'general'} audience engagement`);
  ...
  private getAudienceAdjustments(audience?: string): { requiredSections: string[]; optionalSections: string[] } {
    const adjustments: Record<string, { requiredSections: string[]; optionalSections: string[] }> = {
      beginners: {
      ...
      'decision-makers': {
        requiredSections: ['executive-summary', 'roi-analysis', 'implementation-timeline'],
        optionalSections: ['technical-specifications'],
      },
    };

    return adjustments[audience || 'general'] || { requiredSections: [], optionalSections: [] };
  }

  private getGoalAdjustments(goals?: string[]): { requiredSections: string[]; optionalSections: string[] } {
  ...
  private selectPromptStrategy(contentTypeResult: any, options: any): ContentRoutingConfig['promptStrategy'] {
    if (options.audience === 'experts' || contentTypeResult.contentType === 'TUTORIAL') {
      return 'technical';
    }
    if (contentTypeResult.contentType === 'OPINION' || options.goals?.includes('brand-awareness')) {
      return 'creative';
    }
    if (contentTypeResult.contentType === 'HOWTO' || contentTypeResult.contentType === 'GUIDE') {
      return 'structured';
    }
    return 'conversational';
  }

  private selectSEOLevel(options: any): ContentRoutingConfig['seoOptimization'] {
  ...
  private selectQualityLevel(options: any): ContentRoutingConfig['qualityLevel'] {
    if (options.businessContext || options.goals?.includes('thought-leadership')) {
      return 'expert';
    }
    if (options.goals?.includes('lead-generation') || options.goals?.includes('brand-awareness')) {
      return 'premium';
    }
    return 'standard';
  }

  private combineStrategies(strategies: Array<Partial<ContentRoutingConfig> | undefined>): Partial<ContentRoutingConfig> {
    const combined: Partial<ContentRoutingConfig> = {};
    
    // Take the highest quality level
    const qualityLevels = ['standard', 'premium', 'expert'];
    const maxQuality = strategies.reduce((max, strategy) => {
    ...
    const allRequiredSections = strategies.flatMap(s => s?.requiredSections || []);
    combined.requiredSections = [...new Set(allRequiredSections)];

    return combined;
  }

  private calculatePriorityScore(topicConfig: any, routing: RoutingDecision, globalConfig: any): number {
    let score = 0;
    ...
    // Content type bonus
    if (routing.config.contentType === 'GUIDE' || routing.config.contentType === 'TUTORIAL') {
  ...
  private detectContentSeries(topic: string, existingResults: any[]): string | undefined {
