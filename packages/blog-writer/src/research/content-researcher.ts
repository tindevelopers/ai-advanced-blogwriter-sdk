...
export class ContentResearcher {
   ...
   */
  async research(config: ContentResearchConfig): Promise<ContentResearchResult> {
    ...
    try {
      ...
      const [
        ...
        config.includeCompetitors ? this.analyzeCompetitors(config) : Promise.resolve(undefined),
...
Focus on keywords that would be valuable for content creation and SEO.`;
    ...
    return {
      ...
      ...(config.includeTrends && result.object.trending && {
...
Focus on insights that would help create competitive, differentiated content.`;
    ...
    const topContent: CompetitorContent[] = result.object.topContent.map((content: any) => ({
   ...
   */
  private async generateRecommendations(config: ContentResearchConfig): Promise<{
    structure: string[];
...
Base recommendations on best practices for this topic and target audience.`;
   ...
   */
  async quickResearch(topic: string, options?: TopicResearchOptions): Promise<{
    summary: string;
    keywords: string[];
    questions: string[];
    suggestions: string[];
...
export async function researchKeywords(
  ...
  return [...result.keywords.primary, ...result.keywords.longTail, ...result.keywords.related];
}
